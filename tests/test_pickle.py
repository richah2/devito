...
def test_pickle():
    ...
    assert np.isclose(np.linalg.norm(model.m.data-new_model.m.data), 0)
    assert np.isclose(model.critical_dt, new_model.critical_dt)

    new_model.m.data[:] = new_model.m.data*2
    pkl_model = pickle.dumps(new_model)
    new_model = pickle.loads(pkl_model)
